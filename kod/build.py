...
import os as _os
# Ishodnaya pechatnaya kniga. Yazyk zadaetsya peremennoy LANG_BOOK: en ili es.
# Put mozhno perebit peremennoy SRC_PDF.
_LANG = _os.environ.get('LANG_BOOK', 'en')
SRC = _os.environ.get(
    'SRC_PDF',
    '/home/claude/book-drawings/pak/ishodniki/book %s print.pdf'
    % ('spanish' if _LANG == 'es' else 'eng'))
W, H = 612.0, 792.0
BLACK = (0.0, 0.0, 0.0)
...
STEP_ORDER = {
    ...
    # risuetsya mashina.
    141: [1, 3, 5, 2, 4, 6],
    # Alpaka, stranica 40: nomera 3, 4 i 5 stoyat ne v tom poryadke, v kakom
    # risuetsya figura. Proveryaetsya po seromu sledu: seroe v shage eto
    # risunok predydushchego shaga. Seroe shaga 3 sovpadaet s polnym shagom 5,
    # seroe shaga 4 s shagom 2, seroe shaga 5 s shagom 4.
    39: [1, 2, 4, 5, 3, 6],
    # KNIGA 2. Strekoza, stranica 82: shagi 3 i 4 stoyat ne v tom poryadke.
    # Seroe shaga 3 v tochnosti sovpadaet s polnym shagom 4, a seroe shaga 4
    # s shagom 2. Snachala risuyutsya dva verhnih kryla, potom nizhnee.
    81: [1, 2, 4, 3, 5],
}
...
# Detali, kotorye kniga narisovala i tut zhe poteryala. Klyuch: stranica shagov.
# Znachenie: nomer shaga v knige i uslovie, kakie linii ubrat.
UBRAT = {
    # Mashina, stranica 142. Na shage 5 poyavlyayutsya dve malenkie fary, na
    # sledushchem shage oni ischezayut i vozvrashchayutsya tolko v samom konce,
    # uzhe kak glaza. Rebenok risoval by ih dvazhdy, poetomu na shage 5 ubiraem.
    141: {5: lambda r: r.width < 25 and r.height < 25},
}


def ochistit(spi, steps):
    """Ubiraet poteryannye detali. Nomer shaga zdes knizhnyy, do perestanovki."""
    pr = UBRAT.get(spi)
    if not pr:
        return steps
    out = []
    for k, (r, ps) in enumerate(steps):
        ...
        if f:
            ps = [p for p in ps
                  if vec.col(p) in (vec.GRAY, vec.GRAY2) or not f(pymupdf.Rect(p['rect']))]
        out.append((r, ps))
    return out
