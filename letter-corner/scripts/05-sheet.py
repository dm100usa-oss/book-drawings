...
V=json.load(open(DATA+'/vec3/letters.json')); PPU=V['A']['PPU']
...
def lg(ch,dx,dy,s):
    ...
    for e in L.get('extra',[]):
        if e.get('kind')=='dash':
            ...
            g+=(f'<path d="{d}" fill="none" stroke="#000" stroke-width="0.62" '
...
P.append('</svg>')
svg='\n'.join(P)
open(OUT+'/alphabet-26-pairs.svg','w').write(svg)
cairosvg.svg2pdf(bytestring=svg.encode(), write_to=OUT+'/alphabet-26-pairs.pdf')
cairosvg.svg2png(bytestring=svg.encode(), write_to=OUT+'/alphabet-26-pairs.png', output_width=1600)
