...
MJ=json.load(open(DATA+'/meas2.json')); PPU=MJ['PPU']; WT=MJ['WT']; MT=MJ['MT']
CAP=MJ['CAP']; XH=MJ['XH']
...
FONT=os.path.join(os.path.dirname(DATA),'..','fonts','Quicksand-Variable.ttf')
...
for ch in LET:
    ...
    np.save(fDATA+'/out/{ch}.npy', ink)
    flag='' if len(cs)==EXP[ch] else '   <-- CHECK'
    print('%-2s exp=%d found=%d  %s%s'%(ch,EXP[ch],len(cs),
        ' '.join('%s(%.2f)'%(c['d'],c['iou']) for c in cs),flag))
json.dump(res, open(DATA+'/detect.json','w'), indent=1)
np.save(DATA+'/crops.npy', np.array(list(crops.items()),dtype=object), allow_pickle=True)
...
im.save(DATA+'/digits_montage.png')
